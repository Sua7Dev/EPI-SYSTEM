...
# formulario de registro que ve el superusuario xd
def registro_formulario():
    try:
        ...
        with st.form(key='registro_form'):
            ...
            with col_volver:
                volver_btn = st.form_submit_button(label="Volver atras", type="secondary", width="stretch", icon=":material/arrow_back:",
            ...
            # logica de botones y validaciones
            if registrar_btn:
                if not all([nombre, sexo, nacimiento, nombre_usuario, correo, ci, nacionalidad, contrasena, confirmar_contra, rol]):
                    ...
                    exito = guardar_usuario(
                        ...
                        nombre_usuario=nombre_usuario, #le puse keys para evitar ordenamiento y por si aca las limpieza de casillas 
                        correo=correo,
                        ci=ci,
                    ...
                    if exito:
                        ...
                        st.rerun()
                    #else:
                    #    st.error("Error al registrar usuario. Verifica que el usuario o correo no estén repetidos.", icon=":material/error:")
            if volver_btn:
        ...
        # Asumiendo que la tabla se llama 'usuarios' y las columnas tienen restricciones UNIQUE
        if 'usuarios.nombre_usuario' in error_message:
            ...
            # Mensaje para cualquier otro error de integridad no esperado
            st.error(f"Error de integridad de datos: {e}", icon=":material/error:")
        return


# Ej    ecucion principal
def registro():

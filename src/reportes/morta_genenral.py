...
def exportar_pdf_mortalidad_general(year=None, specific_date=None, start_date=None, end_date=None):
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            cursor.execute("""
                ...
                JOIN departamento_hospital dh ON h.id_hospital = dh.id_hospital
                JOIN departamento d ON dh.id_departamento = d.id_departamento
                WHERE d.nombre = 'Epidemiología'
            """)
            result = cursor.fetchone()
            hospital_nombre = result[0] if result else "Hospital Desconocido"
            departamento_nombre = result[1] if result else "Epidemiología"
    except sqlite3.Error:
        hospital_nombre = "Hospital Dr. Luis Felipe Guevara Rojas"
        departamento_nombre = "Epidemiología"

    direccion_hospital = "El Tigre, Edo. Anzoátegui"
    ...
    pdf.cell(0, 8, departamento_nombre, ln=1, align='J')
    pdf.cell(0, 8, hospital_nombre, ln=1, align='J')
    pdf.set_font("Arial", '', 14)
    pdf.cell(0, 8, direccion_hospital, ln=1, align='J')
    pdf.ln(6)

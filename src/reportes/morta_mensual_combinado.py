...
def exportar_pdf_mortalidad_mensual_combinado(year=None, month=None, specific_date=None, start_date=None, end_date=None):
    try:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            base_query = """
                SELECT m.id_mortaM AS id, m.causas, m.n_casos, m.tasa, m.total, m.fecha_registro_formulario, 'Infantil' AS tipo
                FROM mortalidad_mensual_infantil t
                JOIN mortalidad_mensual m ON t.id_mortaM = m.id_mortaM
                {where_clause}
                UNION
                SELECT m.id_mortaM AS id, m.causas, m.n_casos, m.tasa, m.total, m.fecha_registro_formulario, 'Neonatal' AS tipo
            ...
            where_clause = ""
            if year and month:
                where_clause = "WHERE strftime('%Y-%m', m.fecha_registro_formulario) = ?"
                params = [f"{year}-{month:02d}"] * 2
            elif specific_date:
                where_clause = "WHERE m.fecha_registro_formulario = ?"
                params = [specific_date] * 2
    ...
    pdf.cell(0, 8, f"Reporte de Mortalidad Mensual (Infantil y Neonatal) - {time_frame}", ln=1, align='J')
    pdf.ln(6)
    ...
    if df.empty:
        ...
        tipos = ['Neonatal', 'Infantil']
        for tipo in tipos:
            pdf.set_font("Arial", 'B', 10)
            pdf.cell(0, 6, f"--- Mortalidad {tipo} ---", ln=1, align='L')
            pdf.ln(3)

            df_tipo = df[df['tipo'] == tipo][['causas', 'n_casos', 'tasa', 'total', 'mes']]
            df_grouped = df_tipo.groupby(['mes', df_tipo['causas'].str.lower()]).agg({
                'causas': 'first',
                'n_casos': 'sum',
                'tasa': 'first',
                'total': 'first'
    ...
    pdf.set_title(f"Reporte_Mortalidad_Mensual_Combinado_{datetime.datetime.now().strftime('%d-%m-%Y_%H-%M-%S')}")
